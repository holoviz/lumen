...
class ChatLogs(param.Parameterized):
    ...
    def _serialize_message(self, message: ChatMessage | str) -> dict:
        ...
        elif isinstance(message.object, Markdown):
            return {"type": "markdown", "content": message.object.object}
...
class SQLiteChatLogs(ChatLogs):
    ...
    def __init__(self, **params) -> None:
        ...
        self.cursor.execute(
            ...
            CREATE TABLE IF NOT EXISTS agents (
                ...
                llm_id TEXT REFERENCES llms(llm_id),
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    def _prepare_table_insert(self, parameterized: param.Parameterized, table_name: str) -> dict:
        """
        Prepare data for insertion into a table by extracting columns from the schema.
        Any fields not matching column names are stored in a 'params' JSON column if it exists.
        """
        self.cursor.execute(f"PRAGMA table_info({table_name})")
        table_columns = [row[1] for row in self.cursor.fetchall()]

        spec = parameterized.to_spec()
        data = {}
        for table_column in table_columns:
            if table_column == "params":
                continue
            if table_column.endswith(("_id", "_ids")):
                # table_column = llm_id; key = llm
                key = table_column.split("_id")[0]
                if obj := getattr(parameterized, key, None): # coordinator.llm
                    data[table_column] = obj.hash
                    spec.pop(key)  # clean up {"llm": ...}
                else:
                    data[table_column] = parameterized.hash
            else:
                data[table_column] = json.dumps(serialize_value(spec.pop(table_column, None)))
        data['params'] = json.dumps(serialize_value(spec))
        return data
    ...
    def register_coordinator(
        ...
        try:
            ...
            llm_table_data = self._prepare_table_insert(coordinator.llm, 'llms')
            self.cursor.execute(
                """
                INSERT OR IGNORE INTO llms
                (llm_id, mode, model_kwargs, temperature, params)
                VALUES (:llm_id, :mode, :model_kwargs, :temperature, :params)
                """,
                llm_table_data
            )

            for agent in coordinator.agents:
                agent_table_data = self._prepare_table_insert(agent, 'agents')
                self.cursor.execute(
                    """
                    INSERT OR IGNORE INTO agents
                    (agent_id, name, purpose, requires, provides, prompts,
                     template_overrides, params, tool_ids, llm_id)
                    VALUES (
                        :agent_id, :name, :purpose, :requires, :provides, :prompts,
                        :template_overrides, :params, :tool_ids, :llm_id
                    )
            ...
            for tool in coordinator.tools:
                tool_table_data = self._prepare_table_insert(tool, 'tools')
                self.cursor.execute(
                    """
                    INSERT OR IGNORE INTO tools
                    (tool_id, name, purpose, requires, provides, prompts,
                     template_overrides, params, llm_id)
                    VALUES (
                        :tool_id, :name, :purpose, :requires, :provides, :prompts,
                        :template_overrides, :params, :llm_id
                    )
            ...
            coordinator_table_data = self._prepare_table_insert(coordinator, 'coordinators')
            self.cursor.execute(
                """
                INSERT OR IGNORE INTO coordinators
                (coordinator_id, name, prompts, template_overrides,
                 history, params, agent_ids, tool_ids, llm_id)
                VALUES (
                    :coordinator_id, :name, :prompts, :template_overrides,
                    :history, :params, :agent_ids, :tool_ids, :llm_id
                )
                """,
                coordinator_table_data
            )
            ...
            self.cursor.execute(
                ...
                (session_id, username, user_info, coordinator_id)
                VALUES (
                    :session_id, :username, :user_info, :coordinator_id
                )
                """,
    ...
    def upsert(
        ...
        memory_json = json.dumps(memory.to_spec()) if memory else None
    ...
    def view_messages(self, session_id: str | None = None) -> pd.DataFrame:
        ...
        if session_id == "all":
            ...
            if session_id is None:
                self.cursor.execute(
                    "SELECT session_id FROM messages ORDER BY timestamp ASC LIMIT 1"
                )
