...
Extend `DownloadControls` to create custom source controls:
...
class MyCustomControl(DownloadControls):
    # Define parameters for user inputs
    year = param.Integer(default=2023, bounds=(2010, 2023))
    
    label = '<span class="material-icons">cloud</span> My Data'
    ...
    async def _on_fetch(self, event):
        # Fetch and register data here
        pass
```

### Modern patterns

**Parameter binding with from_param**:

```python
from panel_material_ui import IntSlider, Button, Column

self._year_select = IntSlider.from_param(self.param.year, label="Year")
self._fetch_button = Button(label="Fetch Data", on_click=self._on_fetch)
self._layout = Column(self._year_select, self._fetch_button)
```

**Async data fetching with loading indicator**:

```python
async def _on_fetch(self, event):
    with self._layout.param.update(loading=True):
        await asyncio.sleep(0.01)  # Ensure UI updates
        df = await asyncio.to_thread(fetch_data_from_api, self.year)
        if df is not None and not df.empty:
            await self._add_table(df)
            self.param.trigger("upload_successful")
```

**Simplified DuckDB registration**:

```python
from lumen.util import normalize_table_name

async def _add_table(self, df):
    table_name = normalize_table_name(f"my_data_{self.year}")
...
Use `@param.depends()` to update options when parameters change:

```python
@param.depends("dataset", watch=True)
def _on_dataset_change(self):
    """Update available years when dataset changes."""
    self._year_select.options = self._get_years_for_dataset()
```
...
self._progress_description.object = "Fetching data from API..."
```
